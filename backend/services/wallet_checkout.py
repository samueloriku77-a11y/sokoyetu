...
def process_wallet_checkout(
    db: Session, order_data: schemas.OrderCreate, current_user: models.User
):
    ...
    if wallet.balance < total_payable:
        raise HTTPException(
            400,
            f"Insufficient wallet balance. Required: {total_payable:.2f} KES, Available: {wallet.balance:.2f} KES",
        )
