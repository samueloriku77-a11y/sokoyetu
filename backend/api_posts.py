...
def list_posts(limit: int = 20, offset: int = 0, db: Session = Depends(get_db)):
    ...
    now = time.time()
    if (
        POSTS_COUNT_CACHE["count"] is None
        or (now - POSTS_COUNT_CACHE["ts"]) > POSTS_COUNT_CACHE["ttl"]
    ):
    ...
    posts = (
        ...
        .filter(models.Post.is_hidden == False)
...
def toggle_like(
    ...
    current_user: models.User = Depends(
        require_role("CUSTOMER", "VENDOR", "DRIVER", "ADMIN")
    ),
    db: Session = Depends(get_db),
):
    post = db.query(models.Post).filter(models.Post.id == post_id).first()
